...
const query = `*[_type == "radio" && slug.current == $slug][0]{
  title,
  publishedDate,
  image {
    asset -> {
...
export default function RadioPost(initialData) {
  const { data: { title, publishedDate, image, content, newContent, radioEmbedCode, slug  } } = pageService.getPreviewHook(initialData)()
    ...
    <Layout>
      ...
      <div className="w-full pt-[160px] md:pt-[200px] pb-8 md:pb-8 xl:pb-12 bg-green relative mb-[17vw] md:mb-[15vw]">
        <div className="w-full text-center relative z-10">
          ...
          <span className="text-lg md:text-xl xl:text-2xl text-center block mb-1 md:mb-2 text-off-white">{`${da} ${mo} ${ye}`}</span>
        </div>
              ...
              <div className="w-11/12 md:w-9/12 mx-auto max-w-[1000px]">
                ...
                <div className="w-11/12 md:w-10/12 mx-auto">
                  <div className="content content--fancy mb-10 md:mb-12 xl:mb-16">
                    ...
                    <div className="radio-embed" dangerouslySetInnerHTML={{ __html: radioEmbedCode }}></div>
                  ...
                  <div className="md:flex md:space-x-6">
                    <Link href="/radio">
                      <a className="block md:inline-block font-display bg-pink hover:bg-off-black text-off-white px-8 py-5 text-lg md:text-[2vw] 2xl:text-[30px] uppercase rounded-full leading-none md:leading-none xl:leading-none 2xl:leading-none text-center mb-5 md:mb-0 focus-visible:outline-none focus-visible:border-none focus-visible:ring-4 ring-pink ring-offset-off-white ring-offset-4">Back To All Shows</a>
                    </Link>
...
export async function getStaticPaths() {
  const paths = await pageService.fetchPaths('radio')
  return {
    paths: paths,
    fallback: false,
  };
